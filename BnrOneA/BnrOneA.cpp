...
void BnrOneA::spiConnect(byte sspin)
{
	...
	#ifdef ESP32
	pinMode(RED,OUTPUT); //sspin as output
	pinMode(GREEN,OUTPUT); //sspin as output
	pinMode(BLUE,OUTPUT); //sspin as output
...
/**************************************************************/
#ifdef ESP32
void BnrOneA::setLEDColor(int red,int green,int blue)
{
   ledcWrite(1, red);
   ledcWrite(2, green);
   ledcWrite(3, blue);
